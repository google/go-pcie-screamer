...
func TestDecodeResponseBuffer(t *testing.T) {
	tests := []struct {
		desc string
		res  []byte
		want [][]byte
		...
		{desc: "Multiple TLP packets in a single response buffer",
			res: []byte{
				0xf2, 0xff, 0xff, 0xef, 0xcc, 0xdd, 0xee, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
				0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
				0x00, 0x00, 0x00, 0xe0, 0x4a, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x20, 0x60, 0x00, 0x80, 0x00,
			...
			want: [][]byte{
				{0x4a, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x20, 0x60, 0x00, 0x80, 0x00,
					0x20, 0x74, 0x68, 0x61, 0x6e, 0x20, 0x65, 0x78, 0x70, 0x65, 0x63, 0x74,
					0x65, 0x64, 0x00, 0x61, 0x62, 0x6f, 0x72, 0x74, 0x00, 0x67, 0x72, 0x75,
					0x62, 0x5f, 0x62, 0x69, 0x6f, 0x73, 0x5f, 0x69},
				{0x4a, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x20, 0x60, 0x00, 0x80, 0x00,
					0x20, 0x74, 0x68, 0x61, 0x6e, 0x20, 0x65, 0x78, 0x70, 0x65, 0x63, 0x74,
					0x65, 0x64, 0x00, 0x61, 0x62, 0x6f, 0x72, 0x74, 0x00, 0x67, 0x72, 0x75,
					0x62, 0x5f, 0x62, 0x69, 0x6f, 0x73, 0x5f, 0x69},
	...
	for _, test := range tests {
		t.Logf("Start case: %s", test.desc)
		c := NewTLPController(nil)
		c.readBuffer.Write(test.res)

		err := c.decodeResponseBuffer()
		if err != nil {
			t.Errorf("decodeResponseBuffer(%X, _) = %v, want nil error", test.res, err)
		}

		got := [][]byte{}
		for i := 0; i < len(c.tlpQueue); i++ {
			got = append(got, c.tlpQueue[i].Bytes())
		}

		if diff := cmp.Diff(test.want, got, cmpopts.EquateEmpty()); diff != "" {
			t.Errorf("decodeResponseBuffer() diff -want +got\n%s", diff)
		}
	}
}
